...
*/
// gl_vidsdl.c -- SDL3 GL vid component
...
static display_mode_t fullscreen_modes[MAX_MODE_LIST - 1];
static int num_fullscreen_modes;
...
// These four are used by CheckArrayExtensions.
// glArrayElementEXT, glColorPointerEXT, glVertexPointerEXT are declared
// extern PROC in glquake.h (WIN32 block) and defined here.
// glTexCoordPointerEXT is not in glquake.h and is fully defined here.
...
qboolean VID_SetFullDIBMode (int modenum)
{
	...
	if (!leavecurrentmode)
	{
		// SDL3: fullscreen mode is set via SDL_SetWindowFullscreen with size
	}
	...
	// Create the DIB window
	// SDL3: SDL_WINDOW_FULLSCREEN replaces the removed SDL_WINDOW_FULLSCREEN_DESKTOP flag
	dibwindow = SDL_CreateWindow (
		 "GLQuake",
		 width,
		 height,
...
void	VID_SetPalette (unsigned char *palette)
{
	...
	FILE *f;
	char s[255];
	SDL_Window *hDlg; void *hProgress;
	float gamma;
...
void	VID_Shutdown (void)
{
	...
	{
		vid_canalttab = false;
		hRC = SDL_GL_GetCurrentContext();
     	hWin = SDL_GL_GetCurrentWindow();

     	SDL_GL_MakeCurrent(hWin, NULL);

     	if (hRC)
     	    SDL_GL_DestroyContext(hRC);

		if (modestate == MS_FULLDIB)
			; // SDL3: fullscreen mode is reset by setting windowed again
    ...
    LPARAM  lParam)
{
    ...
    switch (uMsg)
    {
		case WM_KILLFOCUS:
			if (modestate == MS_FULLDIB)
				ShowWindow((HWND)mainwindow, SW_SHOWMINNOACTIVE);
			break;
	...
	// one update sometimes, so we always check all states and look for events
    	...
    	case WM_SIZE:
   	    ...
   	    case WM_CLOSE:
			if (MessageBox ((HWND)mainwindow, "Are you sure you want to quit?", "Confirm Exit",
...
// enumerate >8 bpp modes via SDL3
	originalnummodes = nummodes;
	modenum = 0;
	num_fullscreen_modes = 0;

	displayIndex = SDL_GetPrimaryDisplay();
	if (displayIndex == 0) {
		Con_SafePrintf ("No displays found\n");
		return;
	}

	// SDL3: SDL_GetFullscreenDisplayModes replaces SDL_GetNumDisplayModes +
	// SDL_GetDisplayMode. Returns a NULL-terminated array; free with SDL_free.
	modes = SDL_GetFullscreenDisplayModes(displayIndex, &cmodes);
	if (!modes) {
		Con_SafePrintf ("No displays found\n");
		return;
	}
	for (modenum = 0; modenum < cmodes && num_fullscreen_modes < MAX_MODE_LIST - 1; modenum++) {
			...
			(mode->h <= MAXHEIGHT))
		{
			...
			if (nummodes < MAX_MODE_LIST) {
				...
				modelist[nummodes].bpp = bpp_val;
				sprintf (modelist[nummodes].modedesc, "%dx%dx%d",
						 mode->w, mode->h, bpp_val);
	...
	do
	{
		for (j=0 ; (j<numlowresmodes) && (nummodes < MAX_MODE_LIST) ; j++)
		{
			...
			for (i = 0; i < nummodes; i++) {
				if (modelist[i].width == lowresmodes[j].width &&
					modelist[i].height == lowresmodes[j].height &&
					modelist[i].bpp == bpp) {
					found = 1;
					break;
				}
			...
			if (!found && nummodes < MAX_MODE_LIST) {
				modelist[nummodes].type = MS_FULLDIB;
				...
				sprintf (modelist[nummodes].modedesc, "%dx%dx%d",
						 lowresmodes[j].width, lowresmodes[j].height, bpp);
...
void	VID_Init (unsigned char *palette)
{
	...
	// SDL3: no icon loading in this port
	...
	{
// SDL3: assume RGB mode is always available on modern systems
// Original checked GetDeviceCaps(hdc, RASTERCAPS) & RC_PALETTE
	...
	else
	{
		...
		else
		{
			...
			{
				// SDL3: SDL_GetCurrentDisplayMode takes one arg and returns a pointer
				SDL_DisplayID displayIndex = SDL_GetPrimaryDisplay();
				const SDL_DisplayMode *currentMode = (displayIndex != 0) ?
					SDL_GetCurrentDisplayMode(displayIndex) : NULL;
				if (currentMode) {
					modelist[MODE_FULLSCREEN_DEFAULT].width = currentMode->w;
					modelist[MODE_FULLSCREEN_DEFAULT].height = currentMode->h;
				} else {
					modelist[MODE_FULLSCREEN_DEFAULT].width = 800;
					modelist[MODE_FULLSCREEN_DEFAULT].height = 600;
				}
	...
	DestroyWindow ((HWND)hwnd_dialog);
...
// Set up the OpenGL pixel format via SDL3 GL attributes
// These mirror the original PIXELFORMATDESCRIPTOR settings:
// PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENGL | PFD_DOUBLEBUFFER
// PFD_TYPE_RGBA, 24-bit color, 32-bit z-buffer, no stencil
	...
	baseRC = SDL_GL_CreateContext(mainwindow);
	if (!baseRC)
		Sys_Error ("Could not initialize GL (SDL_GL_CreateContext failed).\n\nMake sure you in are 65535 color mode, and try running -window.");
    if (!SDL_GL_MakeCurrent(mainwindow, baseRC))
